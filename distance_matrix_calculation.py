...
def get_data_set(location_list, travel_mode):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            dist = calculate_time_between_2_places(location_list[i][1], location_list[i][2], location_list[j][1], location_list[j][2],
                                                   travel_mode)
...
def get_display_result_for_last_place_to_destination(last_place_info, destination_info, mode):
    ...
    duration = calculate_time_between_2_places(last_place_info[1],last_place_info[2], destination_info[1],destination_info[2], mode)
...
if __name__ == "__main__":
    ...
    locations = [starting_point, place1, place2, destination]
    mode = "driving"
